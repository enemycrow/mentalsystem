...
$objective_id = (int) ($_GET['objective_id'] ?? 0);
if ($objective_id <= 0) {
...
// Fetch system
$stmt = $db->prepare(
    'SELECT id, purpose, created_at FROM systems WHERE objective_id = :objective_id'
);
...
if (!$system) {
    http_response_code(404);
    echo json_encode(['error' => 'System not found for this objective']);
    exit();
}
...
echo json_encode(['system' => $system]);
